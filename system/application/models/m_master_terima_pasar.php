...
class M_master_terima_pasar extends Model{

		//constructor
		function M_master_terima_pasar() {
			parent::Model();
		}
		...
		$sql="SELECT detail_minta_beli.dminta_produk as dorder_produk,detail_minta_beli.dminta_satuan as dorder_satuan,
				...
				WHERE detail_minta_beli.dminta_master = '".$orderid."'
...
/*			$limit = $sql." LIMIT ".$start.",".$end;
		...
		function get_produk_order_list($order_id,$query,$start,$end){
			...
			if($order_id<>"")
				$sql.=" WHERE produk_id IN(SELECT dminta_produk FROM detail_minta_beli WHERE dminta_master='".$order_id."')";
...
/*			$limit = $sql." LIMIT ".$start.",".$end;
		...
		function get_order_beli_list($filter,$start,$end){
			...
			$sql=  "SELECT 
						...
						supplier_nama, 
						supplier_id, 
						sum(dorder_jumlah) as jumlah_order, 
			...
			*/
			$sql= "
				select
					master_minta_beli.minta_no,
					master_minta_beli.minta_id,
					master_minta_beli.minta_tanggal
				from master_minta_beli
				where master_minta_beli.minta_status = 'Tertutup' and master_minta_beli.minta_jenis = 'Pasar'
			";		
			if ($filter<>""){
				$sql .=eregi("WHERE",$sql)? " AND ":" WHERE ";
				$sql .= " (minta_no LIKE '%".addslashes($filter)."%')";
			}
			/*
...
/*			$limit = $query." LIMIT ".$start.",".$end;
		...
		function detail_detail_terima_beli_list($master_id,$query,$start,$end) {
			$query = "SELECT  distinct dterima_id,dterima_master,dterima_produk,produk_nama,dterima_satuan,jumlah_order, jumlah_sisa,
								dterima_jumlah,harga_satuan,diskon, dterima_keterangan
		...
		function detail_detail_terima_beli_insert($array_dterima_id ,$dterima_master ,$array_dterima_produk ,$array_dterima_satuan ,
												  $array_dterima_jumlah, $array_dterima_keterangan ){

			 $query="";
			 for($i = 0; $i < sizeof($array_dterima_produk); $i++){

				$data = array(
					"dterima_pasar_master"=>$dterima_master,
					"dterima_pasar_produk"=>$array_dterima_produk[$i],
					"dterima_pasar_satuan"=>$array_dterima_satuan[$i],
					"dterima_pasar_jumlah"=>$array_dterima_jumlah[$i],
					"dterima_pasar_keterangan"=>$array_dterima_keterangan[$i]
				);

				if($array_dterima_id[$i]==0){
					$this->db->insert('detail_terima_pasar_beli', $data);
					...
					$this->db->where('dterima_pasar_id', $array_dterima_id[$i]);
					$this->db->update('detail_terima_pasar_beli', $data);
				}
			}
			if($query<>""){
				$sql="DELETE FROM detail_terima_pasar_beli WHERE  dterima_pasar_master='".$dterima_master."' AND
						dterima_pasar_id NOT IN (".$query.")";
				$this->db->query($sql);
			}
		...
		function master_terima_beli_list($filter,$start,$end){
			$query = "
				select 
				`master_terima_pasar_beli`.`terima_pasar_id` AS `terima_id`,
				`master_terima_pasar_beli`.`terima_pasar_no` AS `no_bukti`,
				`master_terima_pasar_beli`.`terima_pasar_order` AS `terima_order`,
				`master_minta_beli`.`minta_no` AS `order_no`,
				`master_minta_beli`.`minta_tanggal` AS `order_tanggal`,
				`master_terima_pasar_beli`.`terima_pasar_supplier` AS `terima_supplier`,
				`master_terima_pasar_beli`.`terima_pasar_surat_jalan` AS `terima_surat_jalan`,
				`master_terima_pasar_beli`.`terima_pasar_pengirim` AS `terima_pengirim`,
				`master_terima_pasar_beli`.`terima_pasar_tanggal` AS `tanggal`,
				`master_terima_pasar_beli`.`terima_pasar_keterangan` AS `terima_keterangan`,
				`master_terima_pasar_beli`.`terima_pasar_status` AS `terima_status`,
				`master_terima_pasar_beli`.`terima_pasar_creator` AS `terima_creator`,
				`master_terima_pasar_beli`.`terima_pasar_date_create` AS `terima_date_create`,
				`master_terima_pasar_beli`.`terima_pasar_update` AS `terima_update`,
				`master_terima_pasar_beli`.`terima_pasar_date_update` AS `terima_date_update`,
				`master_terima_pasar_beli`.`terima_pasar_revised` AS `terima_revised`,
				`master_terima_pasar_beli`.`terima_pasar_carabayar` AS `terima_carabayar`,
				`master_terima_pasar_beli`.`terima_pasar_diskon` AS `terima_diskon`,
				`master_terima_pasar_beli`.`terima_pasar_cashback` AS `terima_cashback`,
				`master_terima_pasar_beli`.`terima_pasar_biaya` AS `terima_biaya`,
				`master_terima_pasar_beli`.`terima_pasar_uangmuka` AS `terima_uangmuka`,
				`master_terima_pasar_beli`.`terima_pasar_bayar` AS `terima_bayar`,
				`master_terima_pasar_beli`.`terima_pasar_total` AS `terima_total`,
				`master_terima_pasar_beli`.`terima_pasar_gudang_id` AS `terima_gudang_id`,
				`gudang`.`gudang_nama` AS `terima_gudang_nama`

				from `master_terima_pasar_beli`
				join `master_minta_beli` on `master_minta_beli`.`minta_id` = `master_terima_pasar_beli`.`terima_pasar_order`
				left join `gudang` on `master_terima_pasar_beli`.`terima_pasar_gudang_id` = `gudang`.`gudang_id`
			";
		...
		function master_terima_beli_create(	$terima_no ,
											$terima_order ,
											$terima_order_id, 
											$terima_supplier ,
											$terima_surat_jalan ,
...
//			$terima_no=$this->m_public_function->get_kode_1('master_terima_beli','terima_no',$pattern,14);
			...
			$pattern="PR/".date("ym",$terima_tanggal_pattern)."-";
			$terima_no=$this->m_public_function->get_kode_1('master_terima_pasar_beli','terima_pasar_no',$pattern,12);

			if ($terima_gudang == 'Gudang Retail'){
				$terima_gudang = 2;
			}
			
			$sql="SELECT minta_id FROM master_minta_beli WHERE minta_id='".$terima_order."'";
			$result=$this->db->query($sql);
			if($result->num_rows()){
				$data = array(
					"terima_pasar_no"=>$terima_no,
					"terima_pasar_order"=>$terima_order,
					"terima_pasar_supplier"=>$terima_supplier,
					"terima_pasar_surat_jalan"=>$terima_surat_jalan,
					"terima_pasar_pengirim"=>$terima_pengirim,
					"terima_pasar_tanggal"=>$terima_tanggal,
					"terima_pasar_keterangan"=>$terima_keterangan,
					"terima_pasar_carabayar"=>$terima_carabayar,
					"terima_pasar_cashback"=>$terima_cashback,
					"terima_pasar_bayar"=>$terima_bayar,
					"terima_pasar_total"=>$terima_total,
					"terima_pasar_diskon"=>$terima_diskon,
					"terima_pasar_biaya"=>$terima_biaya,
					"terima_pasar_uangmuka"=>$terima_uangmuka,
					"terima_pasar_status"=>$terima_status,
					"terima_pasar_gudang_id"=>$terima_gudang,
					"terima_pasar_creator"=>$_SESSION[SESSION_USERID],
					"terima_pasar_date_create"=>date('Y-m-d H:i:s'),
					"terima_pasar_revised"=>0
				);
				if($cetak==1){
					$data['terima_pasar_status'] = 'Tertutup';
				}else{
					$data['terima_pasar_status'] = 'Terbuka';
				}
				$this->db->insert('master_terima_pasar_beli', $data);
