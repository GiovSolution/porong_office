...
class C_master_terima_pasar extends Controller {

	//constructor
	function C_master_terima_pasar(){
		parent::Controller();
		session_start();
		$this->load->model('m_master_terima_pasar', '', TRUE);
	}
	
	//set index
	function index(){
		$this->load->helper('asset');
		$this->load->view('main/v_master_terima_pasar');
	...
	function print_faktur(){
		...
		$faktur=(isset($_POST['faktur']) ? @$_POST['faktur'] : @$_GET['faktur']);
		$opsi="faktur";
        $result = $this->m_master_terima_pasar->get_laporan("","","",$opsi,"",$faktur);
	...
	function print_laporan(){
		...
		$data["data_print"]=$this->m_master_terima_pasar->get_laporan($tgl_awal,$tgl_akhir,$periode,$opsi,$group,"");
		if($opsi=='rekap'){
	...
	function get_order_beli_detail_by_order_id(){
		...
		$result=$this->m_master_terima_pasar->get_order_beli_detail_by_order_id($orderid);
		echo $result;
	}
	...
	function get_produk_list(){
		...
		if($task=='detail')
			$result=$this->m_master_terima_pasar->get_produk_detail_list($master_id,$query,$start,$end);
		elseif($task=='list')
			$result=$this->m_master_terima_pasar->get_produk_all_list($query,$start,$end);
		elseif($task=='selected')
			$result=$this->m_master_terima_pasar->get_produk_selected_list($master_id, $selected_id,$query,$start,$end);
		elseif($task=='order')
			$result=$this->m_master_terima_pasar->get_produk_order_list($order_id,$query,$start,$end);
		echo $result;
	}
	...
	function get_satuan_list(){
		...
		if($task=='detail')
			$result=$this->m_master_terima_pasar->get_satuan_detail_list($master_id);
		elseif($task=='produk')
			$result=$this->m_master_terima_pasar->get_satuan_produk_list($selected_id);
		elseif($task=='selected')
			$result=$this->m_master_terima_pasar->get_satuan_selected_list($selected_id);
		elseif($task=='order')
			$result=$this->m_master_terima_pasar->get_satuan_order_list($order_id);
	...
	function get_bonus_list(){
		...
		$selected_id = isset($_POST['selected_id']) ? @$_POST['selected_id'] : @$_GET['selected_id'];
		if($task=='detail')
			$result=$this->m_master_terima_pasar->get_bonus_detail_list($master_id,$query,$start,$end);
		elseif($task=='list')
			$result=$this->m_master_terima_pasar->get_produk_all_list($query,$start,$end);
		elseif($task=='selected')
			$result=$this->m_master_terima_pasar->get_produk_selected_list($master_id,$selected_id,$query,$start,$end);
	...
	function get_order_beli_list(){
		...
		$result=$this->m_master_terima_pasar->get_order_beli_list($query,$start, $end);
		echo $result;
	}
	
	function get_order_beli_search_list(){
		$result=$this->m_master_terima_pasar->get_order_beli_search_list();
		echo $result;
	}
	
	function get_supplier_search_list(){
		$query = isset($_POST['query']) ? $_POST['query'] : "";
		$result=$this->m_master_terima_pasar->get_supplier_search_list($query);
		echo $result;
	}
	
	function get_terima_gudang_list(){
		$result=$this->m_master_terima_pasar->get_terima_gudang_list();
		echo $result;
	}
	...
	function get_dorder_satuan_by_produkorder(){
		...
		$result=$this->m_master_terima_pasar->get_dorder_satuan_by_produkorder($dorder_master, $dorder_produk);
		echo $result;
	}
	...
	function  detail_detail_terima_bonus_list(){
		...
		$end = (integer) (isset($_POST['limit']) ? @$_POST['limit'] : @$_GET['limit']);
		$master_id = (integer) (isset($_POST['master_id']) ? @$_POST['master_id'] : @$_GET['master_id']);
		$result=$this->m_master_terima_pasar->detail_detail_terima_bonus_list($master_id,$query,$start,$end);
		echo $result;
	}
	//end of handler
	...
	//POST variable here
		...
		$result=$this->m_master_terima_pasar->detail_detail_terima_bonus_insert($array_dtbonus_id ,$dtbonus_master ,
	...
	function  detail_detail_terima_beli_list(){
		...
		$end = (integer) (isset($_POST['limit']) ? @$_POST['limit'] : @$_GET['limit']);
		$master_id = (integer) (isset($_POST['master_id']) ? @$_POST['master_id'] : @$_GET['master_id']);
		$result=$this->m_master_terima_pasar->detail_detail_terima_beli_list($master_id,$query,$start,$end);
		echo $result;
	}
	//end of handler
	...
	function get_master_id(){
		$result=$this->m_master_terima_pasar->get_master_id();
		echo $result;
	}
	//
	...
	//POST variable here
		...
		$dterima_keterangan = $_POST['dterima_keterangan'];
		...
		$array_dterima_keterangan = json_decode(stripslashes($dterima_keterangan));
		
		$result=$this->m_master_terima_pasar->detail_detail_terima_beli_insert($array_dterima_id ,$dterima_master ,$array_dterima_produk ,
																			  $array_dterima_satuan ,$array_dterima_jumlah,$array_dterima_keterangan );
		return $result;
	}
	...
	function check_all_order_done(){
		$master_id = (integer) (isset($_POST['master_id']) ? @$_POST['master_id'] : @$_GET['master_id']);
		$this->m_master_terima_pasar->check_all_order_done($master_id);
	}
	...
	function master_terima_beli_list(){
		...
		$start = (integer) (isset($_POST['start']) ? @$_POST['start'] :@$_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? @$_POST['limit'] : @$_GET['limit']);
		$result=$this->m_master_terima_pasar->master_terima_beli_list($query,$start,$end);
		echo $result;
	}
	...
	function master_terima_beli_update(){
		...
		$result = $this->m_master_terima_pasar->master_terima_beli_update($terima_id ,$terima_no ,$terima_order , $terima_order_id,$terima_supplier ,
	...
	function master_terima_beli_create(){
		...
		$terima_keterangan=str_replace("/(<\/?)(p)([^>]*>)", "",$terima_keterangan);
		$terima_keterangan=stripslashes($terima_keterangan);
		$terima_carabayar=trim(@$_POST["terima_carabayar"]);
		$terima_carabayar=str_replace("/(<\/?)(p)([^>]*>)", "",$terima_carabayar);
		$terima_cashback=trim(@$_POST["terima_cashback"]);
		$terima_total=trim(@$_POST["terima_total"]);
		$terima_bayar=trim(@$_POST["terima_bayar"]);
		$terima_diskon=trim(@$_POST["terima_diskon"]);
		$terima_biaya=trim(@$_POST["terima_biaya"]);
		$terima_uangmuka=trim(@$_POST["terima_uangmuka"]);
		$terima_status=trim(@$_POST["terima_status"]);
		$cetak=trim(@$_POST["cetak"]);
		
		$result=$this->m_master_terima_pasar->master_terima_beli_create($terima_no ,
																		$terima_order ,
																		$terima_order_id, 
																		$terima_supplier ,
																		$terima_surat_jalan ,
	...
	function master_terima_beli_delete(){
		...
		$result=$this->m_master_terima_pasar->master_terima_beli_delete($pkid);
		echo $result;
	}
	...
	function master_terima_beli_search(){
		...
		$start = (integer) (isset($_POST['start']) ? @$_POST['start'] : @$_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? @$_POST['limit'] : @$_GET['limit']);
		$result = $this->m_master_terima_pasar->master_terima_beli_search($terima_id ,$terima_no ,$terima_order ,$terima_supplier ,
	...
	function master_terima_beli_print(){
		...
		$data["data_print"]  = $this->m_master_terima_pasar->master_terima_beli_print($terima_id ,$terima_no ,$terima_order ,$terima_supplier ,
																		$terima_surat_jalan ,$terima_pengirim ,$terima_tgl_awal, 
	...
	function master_terima_beli_export_excel(){
		...
		$query = $this->m_master_terima_pasar->master_terima_beli_export_excel($terima_id ,$terima_no ,$terima_order ,$terima_supplier ,
																			  $terima_surat_jalan ,$terima_pengirim ,$terima_tgl_awal ,
																			  $terima_tgl_akhir, $terima_keterangan ,$terima_status,$option,$filter,$terima_gudang);
