...
class C_master_serah_bahan extends Controller {

	//constructor
	function C_master_serah_bahan(){
		parent::Controller();
		session_start();
		$this->load->model('m_master_serah_bahan', '', TRUE);
	...
	function index(){
		...
		$this->load->view('main/v_master_serah_bahan');
	}
	...
	function print_laporan(){
		...
		$data["data_print"]=$this->m_master_serah_bahan->get_laporan($tgl_awal,$tgl_akhir,$periode,$opsi,$group,$bulan,$tahun);
	...
	function get_gudang_tujuan_list(){
		$result=$this->m_master_serah_bahan->get_gudang_tujuan_list();
		echo $result;
	}

	function get_gudang_asal_list(){
		$result=$this->m_master_serah_bahan->get_gudang_asal_list();
		echo $result;
	}
	...
	function get_produk_list(){
		...
		if($task=='detail')
			$result=$this->m_master_serah_bahan->get_produk_detail_list($master_id,$query,$start,$end);
		elseif($task=='list')
			$result=$this->m_master_serah_bahan->get_produk_all_list($query,$start,$end);
		elseif($task=='selected')
			$result=$this->m_master_serah_bahan->get_produk_selected_list($master_id,$selected_id,$query,$start,$end);
		elseif($task=='op_last_price')
			$result=$this->m_master_serah_bahan->get_op_last_price($supplier_id);
		elseif($task=='produksi')
			$result=$this->m_master_serah_bahan->get_produk_pp_list($produksi_id,$query,$start,$end);
		echo $result;
	}
	//
	...
	function get_satuan_list(){
		...
		if($task=='detail')
			$result=$this->m_master_serah_bahan->get_satuan_detail_list($master_id);
		elseif($task=='produk')
			$result=$this->m_master_serah_bahan->get_satuan_produk_list($selected_id);
		elseif($task=='selected')
			$result=$this->m_master_serah_bahan->get_satuan_selected_list($selected_id);
		elseif($task=='order')
			$result=$this->m_master_serah_bahan->get_satuan_minta_list($minta_id);
	...
	function get_satuan_bybahan_jadi_list(){
		...
		$result = $this->m_master_serah_bahan->get_satuan_bybahan_jadi_list($query,$produk_id);
		echo $result;
	}
	...
	function get_satuan_byproduksi_jadi_list(){
		$query = (integer) (isset($_POST['query']) ? $_POST['query'] : 0);
		$produk_id = (integer) (isset($_POST['produk_id']) ? $_POST['produk_id'] : 0);
		$result = $this->m_master_serah_bahan->get_satuan_byproduksi_jadi_list($query,$produk_id);
		echo $result;
	}
	...
	function list_history_bahan_produksi(){
		...
		$end = (integer) (isset($_POST['limit']) ? $_POST['limit'] : $_GET['limit']);
		$master_id = (integer) (isset($_POST['master_id']) ? $_POST['master_id'] : $_GET['master_id']);
		$result=$this->m_master_serah_bahan->detail_bahan_jadi_produksi_list($master_id,$query,$start,$end);
		echo $result;
	}
	...
	function detail_serah_bahan_list(){
		...
		$end = (integer) (isset($_POST['limit']) ? $_POST['limit'] : $_GET['limit']);
		$master_id = (integer) (isset($_POST['master_id']) ? $_POST['master_id'] : $_GET['master_id']);
		$result=$this->m_master_serah_bahan->detail_serah_bahan_list($master_id,$query,$start,$end);
		echo $result;
	}
	...
	function detail_jual_kwitansi_purge(){
		$master_id = (integer) (isset($_POST['master_id']) ? $_POST['master_id'] : $_GET['master_id']);
		$result=$this->m_master_serah_bahan->detail_jual_kwitansi_purge($master_id);
	}
	//eof
	...
	function get_no_permintaan_produksi_list(){
		...
		$start = (integer) (isset($_POST['start']) ? @$_POST['start'] : @$_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? @$_POST['limit'] : @$_GET['limit']);
		$result=$this->m_master_serah_bahan->get_no_permintaan_produksi_list($query,$start, $end);
		echo $result;
	}
	...
	function get_item_detail_by_produksi_id(){
		$produksi_id = isset($_POST['produksi_id']) ? @$_POST['produksi_id'] : @$_GET['produksi_id'];
		$result=$this->m_master_serah_bahan->get_item_detail_by_produksi_id($produksi_id);
		echo $result;
	}

	//get master id, note: not done yet
	function get_master_id(){
		$result=$this->m_master_serah_bahan->get_master_id();
		echo $result;
	}
	//
	...
	function get_action(){
		...
		switch($task){
			case "LIST":
				$this->penyerahan_bahan_list();
				break;
			case "UPDATE":
				$this->penyerahan_bahan_update();
				break;
			case "BATAL":
				$this->penyerahan_bahan_batal();
				break;
			case "CREATE":
				$this->penyerahan_bahan_create();
				break;
			case "CEK":
				$this->penyerahan_bahan_pengecekan();
				break;
			case "DELETE":
				$this->penyerahan_bahan_delete();
				break;
			case "SEARCH":
				$this->penyerahan_bahan_search();
				break;
			case "DDELETE":
				$this->detail_lcl_delete();
				break;
			case "DETAIL_DELETE":
				$this->detail_serahbahan_delete();
				break;
			case "PRINT":
				$this->penyerahan_bahan_print();
				break;
			case "EXCEL":
				$this->penyerahan_bahan_excel();
				break;
			default:
				echo "{failure:true}";
				break;
	...
	function penyerahan_bahan_list(){
		
		$query = isset($_POST['query']) ? $_POST['query'] : "";
		$start = (integer) (isset($_POST['start']) ? $_POST['start'] : $_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? $_POST['limit'] : $_GET['limit']);
		$result=$this->m_master_serah_bahan->penyerahan_bahan_list($query,$start,$end);
		echo $result;
	}

	function penyerahan_bahan_pengecekan(){
	...
	function penyerahan_bahan_update(){
		//POST variable here
		...
		$result = $this->m_master_serah_bahan->penyerahan_bahan_update($serah_id, $serah_no, $serah_tanggal, $serah_keterangan, $serah_status, $serah_produksi,
			$array_dserah_id, $dserah_master, $array_dserah_produk, $array_dserah_satuan, $array_dserah_jumlah, $array_dserah_keterangan, $cetak);
		echo $result;
	}
	
	function penyerahan_bahan_batal(){
		//POST variable here
		...
		$result = $this->m_master_serah_bahan->penyerahan_bahan_batal($kwitansi_id ,$fcl_status ,$kwitansi_update );
		echo $result;
	}
	...
	function penyerahan_bahan_create(){
		...
		$result=$this->m_master_serah_bahan->penyerahan_bahan_create($serah_no, $serah_tanggal, $serah_keterangan, $serah_status, $serah_produksi,
	...
	function penyerahan_bahan_delete(){
		$ids = $_POST['ids']; // Get our array back and translate it :
		$pkid = json_decode(stripslashes($ids));
		$result=$this->m_master_serah_bahan->penyerahan_bahan_delete($pkid);
		echo $result;
	}

	//function for advanced search
	function penyerahan_bahan_search(){
		//POST varibale here
		...
		$start = (integer) (isset($_POST['start']) ? $_POST['start'] : $_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? $_POST['limit'] : $_GET['limit']);
		$result = $this->m_master_serah_bahan->penyerahan_bahan_search($kwitansi_no ,$kwitansi_cust , $kwitansi_tanggal_start, $kwitansi_tanggal_end,$kwitansi_keterangan ,$kwitansi_status , $dfcl_status, $final_status, $start,$end);
		echo $result;
	}

	function detail_lcl_delete(){
        $dfcl_id = trim(@$_POST["dfcl_id"]); // Get our array back and translate it :
		$result=$this->m_master_serah_bahan->detail_lcl_delete($dfcl_id);
		echo $result;
    }

	function detail_serahbahan_delete(){
        $dserah_id = trim(@$_POST["dserah_id"]); // Get our array back and translate it :
		$result=$this->m_master_serah_bahan->detail_serahbahan_delete($dserah_id);
		echo $result;
    }


	function penyerahan_bahan_print(){
  		//POST varibale here
		$kwitansi_no=trim(@$_POST["kwitansi_no"]);
		$kwitansi_no=str_replace("/(<\/?)(p)([^>]*>)", "",$kwitansi_no);
		$kwitansi_no=str_replace("'", "''",$kwitansi_no);
		...
		$data["data_print"] = $this->m_master_serah_bahan->penyerahan_bahan_print($kwitansi_no ,$kwitansi_cust ,$kwitansi_keterangan ,$kwitansi_status ,$option,$filter);
	...
	function print_paper(){
		...
		$result = $this->m_master_serah_bahan->print_paper($kwitansi_id);
		$rs=$result->row();
		$result_cara_bayar = $this->m_master_serah_bahan->cara_bayar($kwitansi_id);
	...
	function penyerahan_bahan_excel(){
		//POST varibale here
		$kwitansi_no=trim(@$_POST["kwitansi_no"]);
		$kwitansi_no=str_replace("/(<\/?)(p)([^>]*>)", "",$kwitansi_no);
		$kwitansi_no=str_replace("'", "''",$kwitansi_no);
		...
		$query = $this->m_master_serah_bahan->penyerahan_bahan_excel($kwitansi_no ,$kwitansi_cust ,$kwitansi_keterangan ,$kwitansi_status ,$option,$filter);
